function [clean_ind] = dbscanr(xy,k,Eps,sh,rsh,s,delta,rate)
...
%           1...65534 when part of a cluster (core or border)
    ...
    while any(class==0)
        ...
        if rho(idx)>=rsh %core point
            cluster = cluster+1;
            class(idx)=cluster;
            idxs(class(idxs)>1)=[]; %no need to redo already allocated points
            class = expandcluster(xy,tree,class,Eps,k,idxs,cluster,rho,rsh);
...
function [class] = expandcluster(xy,tree,class,Eps,k,idxs,cluster,rho,rsh)
...
%else add the borderpoint    
    ...
    for ct = 1:length(idxs) %first one is self
        ...
        if rho(idxs(ct))>=rsh %core point
            idxs2(class(idxs2)>1)=[]; %no need to redo already allocated points (including self)
            class = expandcluster(xy,tree,class,Eps,k,idxs2,cluster,rho,rsh);
